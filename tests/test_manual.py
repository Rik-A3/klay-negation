...
def test_node_equality():
    ...
    assert c.or_node([l1, l2]) != c.or_node([l1, l2], negate=True)
    assert c.and_node([l1, l2]) != c.and_node([l1, l2], negate=True)
...
def test_nor_node():
    c = klay.Circuit()
    l1, l2 = c.literal_node(1), c.literal_node(-2)
    or_node = c.or_node([l1, l2], negate=True)
    c.set_root(or_node)
...
def test_nand_node():
    c = klay.Circuit()
    l1, l2 = c.literal_node(1), c.literal_node(-2)
    and_node = c.and_node([l1, l2], negate=True)
    c.set_root(and_node)

    m = c.to_torch_module(semiring='real')
    weights = torch.tensor([0.4, 0.8])
    assert m(weights) == 1 - 0.4 * (1 - 0.8)
